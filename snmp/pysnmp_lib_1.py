...
class _Gen(pysnmp_mib):
    ...
    def _Gen_bulk(self, target_IP, target_port):
        ...
        errorIndication, errorStatus, errorIndex, varBindTable = self.cmdgen.bulkCmd(
            ...
            oid_id,  # <----------------- doesn't seem perfect either
            lexicographicMode=True,
            maxRows=2500,  # <-------------------- can't leave it out
            ignoreNonIncreasingOid=True,
            lookupNames=True,
            lookupValues=True
        )
...
def _run_walk(target_IP, target_port):
        ...
        if cm is cmd[0]:
            return walk._Gen_bulk(target_IP, target_port)
        elif cm is cmd[1]:
            return walk._Gen_next(target_IP, target_port)
        elif cm is cmd[2]:
            return walk._Gen_get(target_IP, target_port)
