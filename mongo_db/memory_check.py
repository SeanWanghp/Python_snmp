...
class e7_telnet(_Log):
    ...
    def telnet_e7(self, tn, _host, _port, _type=None):
        ...
        if tn is None:
            ...
            self._lg(self.Tel.read_until(b": " or b"# " or b"~ ", 10))
        else:
            pass
        return self.Tel
...
class _Login(object):
    ...
    def login(self):
        if self._type is 'AXOS':
            self.session.write(b"root" + self.enter)
            self.res += self.session.read_until(b": ")
            self.session.write(b"root" + self.enter)
            self.res += self.session.read_until(b"# ")
            self.session.write(b"pwd" + self.enter)
            self.res += self.session.read_until(b"# ")
    ...
    def cli_command(self, command):
        ...
        for cli in command.split('\n'):
            if not cli.strip().startswith('#'):
                self.session.write(command.strip().encode('ascii') + self.enter)
                self.res = self.session.expect([b'# ', b'~#'])
                print(self.res[2].decode('utf-8'))
                self.resu += str(self.res[2])
            else:
...
class _Cli(e7_telnet, Para, _Log, _Login, _Command, _Mongogo, _Matlibplt):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        '''
        logging.warning('AXOS card CPU&MEMORY is checking in process, please waiting...................')
    ...
    def __del__(self):
        ...
        close the session
        '''
        self.session.write(b'exit' + self.enter)
        try:
            self.session.close()
...
if __name__ == "__main__":
    if hasattr(_Cli, 'enter') is False:
        getattr(_Cli, 'enter', setattr(_Cli, 'enter', b'\n'))
        # sleep(600)
    card_ip = '10.245.46.207'
    module_a = 'lmd'
    module_b = 'halm'
